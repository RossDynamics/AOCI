...
%that integrates them to the line segment between L4 and the secondary.
...
%We integrate initial conditions backwards to the interception circle where
%they can be sorted.
...
c = cs(c,'s.i.odeopts',odeset('Events', @(t,y)interceptL4Line(t,y),...
                              'RelTol',3e-14,'AbsTol',1e-15));
...
parfor i = 1:numel(theta)
    ...
    timesunfd = linspace(solunfdEnd{i}.x(1),solunfdEnd{i}.x(end),1000);
    ...
    timesdnfd = linspace(soldnfdEnd{i}.x(1),soldnfdEnd{i}.x(end),1000);
...
imagesc([2 20], [0,1],flipud(cdata))
hold on;

%We take special note of trajectories whose arguments of perihelion are
%close to those used in the arches of chaos computations
aocomega = deg2rad(2.73833e2-360)

%We plot the eccentricity plots:
[apufd,epufd,omegaufd] = getae(endufd,1,L,c);
plot(apufd*LinAU,epufd,'c.')
[apdfd,epdfd,omegadfd] = getae(enddfd,1,L,c);
plot(apdfd*LinAU,epdfd,'r.')

%We sort by which omegaufd's are within a certain range of the aoc value

% tolerance = deg2rad(30);
% 
% closetoaocu = abs(omegaufd - aocomega) < tolerance;
% plot(apufd(closetoaocu)*LinAU,epufd(closetoaocu),'co')
% 
% closetoaocd = abs(omegadfd - aocomega) < tolerance;
% plot(apdfd(closetoaocd)*LinAU,epdfd(closetoaocd),'ro')

xlabel('Post-encounter semi-major axis in AU (b = +, r = -)')
ylabel('Post-encounter eccentricity')
axis([2 20 0 1])
set(gca,'ydir','normal');
